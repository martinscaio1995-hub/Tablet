...
## Rodar no tablet e comandar pelo celular

A ideia é o tablet ficar ligado rodando o pipeline (e o agendador), e o
celular só entrar por SSH para disparar e acompanhar.

```sh
bash setup.sh
```

Rode isso **no Termux do tablet**, não dentro do proot. O script instala
e configura o servidor SSH na porta 8022, autoriza a chave do celular,
supervisiona o serviço e o sobe junto com o boot. É idempotente: rodar de
novo é seguro.

| Comando | O que faz |
|---|---|
| `bash setup.sh` | Instala e configura o acesso SSH |
| `bash setup.sh --chave "ssh-ed25519 ..."` | Autoriza a chave do celular |
| `tablet-status` | Mostra IP, porta, estado do sshd e sessões tmux |

Passo a passo do lado do celular (chaves, `tmux`, o que fazer quando cai):
[`docs/conectar-do-celular.md`](docs/conectar-do-celular.md).

Instalação do Termux e do ambiente Python no tablet:
[`docs/termux-setup.md`](docs/termux-setup.md).
...
setup.sh                   -> configura o acesso SSH ao tablet
bin/tablet-status          -> estado da conexão com o tablet
